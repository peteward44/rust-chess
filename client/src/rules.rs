...
impl Rules {
	...
	fn gen_move(vec: &mut Vec<Move>, board_state: &BoardState, dx: usize, dy: usize, piece: &BoardPiece) -> bool {
	...
	fn gen_diagonals(mut vec: &mut Vec<Move>, board_state: &BoardState, x: usize, y: usize, piece: &BoardPiece) {
		let mut dx = x - 1;
		let mut dy = y - 1;
		while dx >= 0 && dy >= 0 {
	...
	fn gen_straights( mut vec: &mut Vec<Move>, board_state: &BoardState, x: usize, y: usize, piece: &BoardPiece) {
		for dx in (0..x).rev() {
	...
	pub fn get_piece_possible_moves(
		...
		match piece.piece {
			...
			consts::PieceType::KNIGHT => {
				Rules::gen_move( &mut vec, &board_state, x + 1, y + 3, &piece );
				Rules::gen_move( &mut vec, &board_state, x - 1, y + 3, &piece );
				Rules::gen_move( &mut vec, &board_state, x + 1, y - 3, &piece );
				Rules::gen_move( &mut vec, &board_state, x - 1, y - 3, &piece );
				
				Rules::gen_move( &mut vec, &board_state, x + 3, y + 1, &piece );
				Rules::gen_move( &mut vec, &board_state, x - 3, y + 1, &piece );
				Rules::gen_move( &mut vec, &board_state, x + 3, y - 1, &piece );
				Rules::gen_move( &mut vec, &board_state, x - 3, y - 1, &piece );
			}
